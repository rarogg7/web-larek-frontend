...
// Интерфейс для элемента корзины (DOM)
...
export class BasketItemView implements IBasketItem {
  ...
  constructor(
    ...
    if (!this.index || !this.title || !this.price || !this.btnDelete) {
...
export class BasketView implements IBasket {
  ...
  constructor(
    ...
    if (!this.title || !this.button || !this.list || !this.totalPrice || !this.headerBtn || !this.headerCount) {
  ...
  updateBasketView(basketModel: BasketModel, cardBasketTemplateElement: HTMLTemplateElement) {
    ...
    const items = basketModel.cardsList.map((card: ICard, index: number) => {
      const basketItem = new BasketItemView(cardBasketTemplateElement, this.events, {
